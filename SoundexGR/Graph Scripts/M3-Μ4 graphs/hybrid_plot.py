...
plt.scatter(x, pivot_df["Hybrid_i-ii_fixedK"], color='orange', label="Hybrid i-ii (K = 1.5)", s=70)
plt.scatter(x, pivot_df["Hybrid_i-ii_K"], color='blue', label="Hybrid i-ii (K calculated)", s=70)

# Κάθετες γραμμές για να δείχνουν τη διαφορά
for i, dataset in enumerate(pivot_df.index):
    y1 = pivot_df.loc[dataset, "Hybrid_i-ii_fixedK"]
    y2 = pivot_df.loc[dataset, "Hybrid_i-ii_K"]
    plt.plot([i, i], [y1, y2], color='gray', linestyle='--', alpha=0.7)

# Ετικέτες και ρυθμίσεις
plt.title("F-Score Difference per Dataset (Hybrid i-ii Methods)", fontsize=13)
plt.xlabel("Dataset")
plt.ylabel("F-Score")
